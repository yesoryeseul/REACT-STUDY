...
const Pagination = () => {
  ...
  */
  ...
  const onChangePage = (newPage) => {
    // 리팩토링 navigate -> setSearchParams 으로 통일
    setSearchParams({ page: newPage, per_page: perPage, sort });
  };

  // 5. < > 페이지로 이동
  const onPrevArrow = () => {
    const prevPage = page > 1 ? page - 1 : 1;
    setSearchParams({ page: prevPage, per_page: perPage, sort });
  };

  const onNextArrow = () => {
    const nextPage = page < pages ? page + 1 : pages;
    setSearchParams({ page: nextPage, per_page: perPage, sort });
  };
